...
class Persona{
    ...
    int edad_;
    public:
    Persona(const std::string & dni, const std::string & nombre="", const std::string & apellidos ="", const std::string & direccion="", \
    ...
    inline void setEdad(int edad){if (edad >= 0) {edad_ = edad;}}
    inline std::string getApellidosyNombre()const{return getApellidos() +", "+ getNombre();}
    inline bool getMayor(){if (edad_ < 18){return false;} return true;}
    };
